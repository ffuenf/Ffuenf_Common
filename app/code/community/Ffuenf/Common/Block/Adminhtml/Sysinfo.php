...
class Ffuenf_Common_Block_Adminhtml_Sysinfo extends Mage_Adminhtml_Block_Template
{
    ...
    protected function _prepareLayout()
    {
        ...
        $accordion->addItem('general', array(
            ...
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_debug_section')->setDebugArea('general')->toHtml()
        ));
        $accordion->addItem('stores', array(
            'title'     => Mage::helper('ffuenf_common')->__('Stores'),
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_sysinfo_section_table')->setDebugArea('stores')->toHtml()
        ));
        $accordion->addItem('magento_general', array(
            'title'     => Mage::helper('ffuenf_common')->__('Magento Settings'),
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_sysinfo_section_table')->setDebugArea('magento_general')->toHtml()
        ));
        $accordion->addItem('cronjobs', array(
            'title'     => Mage::helper('ffuenf_common')->__('Cronjobs'),
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_sysinfo_section_table')->setDebugArea('cronjobs')->setShowKeys(false)->toHtml()
        ));
        $accordion->addItem('event_observers', array(
            'title'     => Mage::helper('ffuenf_common')->__('Event Observers'),
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_sysinfo_section')->setDebugArea('event_observers')->toHtml()
        ));
        $accordion->addItem('magento_extensions', array(
            'title'     => Mage::helper('ffuenf_common')->__('Installed Magento extensions'),
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_sysinfo_section')->setDebugArea('magento_extensions')->toHtml()
        ));
        $accordion->addItem('php_modules', array(
            'title'     => Mage::helper('ffuenf_common')->__('Installed PHP modules'),
            'content'   => $this->getLayout()->createBlock('ffuenf_common/adminhtml_sysinfo_section')->setDebugArea('php_modules')->toHtml()
        ));
